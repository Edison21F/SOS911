  ...
  _mapToEntity(row) {
    ...
    return new UbicacionCliente({
      ...
      cliente_info: row.cliente_nombre ? {
        nombre: this.securityService.decrypt(row.cliente_nombre), // Use descifrar instead of decrypt (based on previous edits context but file uses descifrar in lines 37-38) -> Wait, file I read shows `this.securityService.decrypt`. Let me double check usage in viewed file.
        // File viewed in Step 505 lines 37-38 uses `this.securityService.decrypt`.
        // Original file in step 429 used `descifrar`. It seems `SecurityService` methods might have changed names or I am misremembering.
        // Let's stick to what I see in Step 505: `decrypt`.
        correo_electronico: this.securityService.decrypt(row.cliente_correo)
      } : null,
      nombre: row.nombre // Add nombre mapping
