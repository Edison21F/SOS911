...
ubicacionClienteCtl.createClientLocation = async (req, res) => {
  ...
  const { clienteId, latitud, longitud, nombre } = req.body;
  logger.info(`[UBICACIONES_CLIENTES] Creación (Hexagonal): clienteId=${clienteId}, lat=${latitud}, long=${longitud}, nombre=${nombre}`);
